...
def main(username, password, venue, download_all, download_pdfs, venueid):
    try:
    ...
    for submission in tqdm(submissions):
        ...
        paper = {
            "id": int(str(venueid)+str(submission.number)),  # len(papers)+1,
            "title": get_content_from(submission, "title"),
            "authors": authors,
            "abstract": abstract,
            "file": str(venueid)+str(submission.number) + ".pdf",  # str(len(papers)+1) + ".pdf",
        ...
        if download_pdfs:
            ...
            with open(
                ...
                os.path.join(papers_folder, str(paper["file"])), "wb"
...
if __name__ == "__main__":
    ...
    #venueid = 'EAMT.org/2024/Products_Projects_Track'
    venueids = {
            100:'EAMT.org/2024/Implementations_Case_Studies_Track',
            200:'EAMT.org/2024/Research_Translators_Users_Track',
            300:'EAMT.org/2024/Technical_Track',
            400:'EAMT.org/2024/Thesis_Award',
            500:'EAMT.org/2024/Tutorials_Track',
            }
    all_papers = []
    for venueid in venueids: 
        papers = main(args.username, args.password, venueids[venueid], args.all, args.pdfs, venueid)
        all_papers += papers
    yaml.dump(all_papers, open("papers.yml", "w"), allow_unicode=True)
