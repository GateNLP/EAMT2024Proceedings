...
acl_name = 'EAMT.org/2024' if len(sys.argv)<=3 else sys.argv[3]
...
to_exlude = ['EAMT.org/2024/Products_Projects_Track']
raw_all_groups = client_acl.get_all_groups(prefix = f"{acl_name}/.*")

all_groups = []
for item in raw_all_groups:
    if item.domain not in to_exlude:
